import {
  ...
  export class TicTacToe extends SmartContract {
    ...
    constructor(alice: PubKey, bob: PubKey) {
      ...
      this.board = fill(BigInt(Player.EMPTY), 9);
    }
    ...
    public move(n: bigint, sig: Sig) {
      ...
      assert(
        this.board[Number(n)] === BigInt(Player.EMPTY),
        `board at position ${n} is not empty: ${this.board[Number(n)]}`
      );
      let play = this.isAliceTurn ? BigInt(Player.ALICE) : BigInt(Player.BOB);
    ...
    full(): boolean {
      let full = true;
      for (let i = 0; i < 9; i++) {
        full = full && this.board[i] !== BigInt(Player.EMPTY);
      }
      return full;
    }
    ...
    static buildTxForMove(
      ...
      const play = current.isAliceTurn ? BigInt(Player.ALICE) : BigInt(Player.BOB);
